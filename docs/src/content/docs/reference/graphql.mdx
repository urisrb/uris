...
| Field | Grant | |
| --- | --- | --- |
| `tenant` | `things:read` | who answered |
| `thing(id:)` | `things:read` | one thing |
| `things(kind:, resourceId:, after:, limit:)` | `things:read` | a page of the catalog |
| `search(query:, kind:, limit:)` | `things:read` | the index, clamped to 1–200 |
| `kinds` | `things:read` | counts grouped by kind |
| `resources` | `resources:read` | active resources, ordered by type and key |
| `runs(kind:, status:, after:, limit:)` | `things:read` | a page of runs |
| `mergeProposals(status:, after:, limit:)` | `things:read` | open proposals by default |
| `auditEvents(action:, status:, subject:, after:, limit:)` | `things:read` | a page of the audit log |
...
| Field | Grant |
| --- | --- |
| `analyzeThing` `mergeThings` `splitReference` `proposeMerges` `settleMergeProposal` | `things:write` |
| `exportThings` `cancelRun` | `things:write` |
| `syncResource` `checkResource` `setDefaultStorage` `setSyncInterval` | `resources:command` |

`exportThings` and `cancelRun` take `things:write` here and `resources:command` over
[MCP](/reference/tools/). Merging and dedupe are reachable here and nowhere else.

### Subscriptions

`thingAnalyzed`, triggered by `Thing#announce_analyzed!` both globally and per thing id.
...
same two commands before the typecheck, because `schema.graphql` and the generated types are build
