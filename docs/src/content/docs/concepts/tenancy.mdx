...
Included by `Thing`, `ThingReference`, `Resource`, `Run`, `Gate`, `AuditEvent`, `MergeProposal` and
`ResourceBlob`.
...
`test/models/tenant_isolation_test.rb` asserts RLS holds with `Thing.unscoped`, that a cross-tenant
...
```
things_development           the shared alias
things_development_t3        tenant 3's filtered alias
things_development_v2026…    the concrete versioned index
```
...
`test/integration/graphql_tenancy_test.rb` asserts that a thing id from another tenant resolves to
