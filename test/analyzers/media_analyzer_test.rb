...
class MediaAnalyzerTest < ActiveSupport::TestCase
  ...
  test "a recording is its own kind rather than an anonymous file" do
    ...
    Tenant.switch(@tenant) do
      assert_equal "audio", item("tone.m4a").kind
      assert_equal "video", item("clip.mp4").kind
    end
  end

  test "both kinds reach the media analyzer" do
    Tenant.switch(@tenant) do
      assert_instance_of Analyzer::Media, Analyzer.for(item("tone.m4a"))
      assert_instance_of Analyzer::Media, Analyzer.for(item("clip.mp4"))
    end
  end
  ...
  test "an audio file yields its length and its streams" do
    ...
    Tenant.switch(@tenant) do
      probe = reference("tone.m4a").analysis.dig("steps", "probe", "result")
  ...
  test "a video yields its picture as well as its sound" do
    ...
    Tenant.switch(@tenant) do
      streams = reference("clip.mp4").analysis.dig("steps", "probe", "result", "streams")
  ...
  test "with no model configured the recording is still catalogued, and says why it is silent" do
    ...
    Tenant.switch(@tenant) do
      steps = reference("tone.m4a").analysis.fetch("steps")
  ...
  test "a model that was named but is not there is refused by name" do
    ...
    Tenant.switch(@tenant) do
      message = reference("tone.m4a").analysis.dig("steps", "transcript", "error", "message")
  ...
  test "the length reaches the prompt, so a summary can say how long it runs" do
    ...
    Tenant.switch(@tenant) do
      analyzer = Analyzer::Media.new(item("clip.mp4"))
      analyzer.instance_variable_set(:@reference, reference("clip.mp4"))
  ...
  test "what was said becomes text, and the text becomes searchable" do
    ...
    Tenant.switch(@tenant) do
      transcript = reference("standup.m4a").analysis.dig("steps", "transcript", "result")
  ...
  test "a recording with no sound in it is not silently reported as heard" do
    ...
    Tenant.switch(@tenant) do
      steps = reference("silent.mp4").analysis.fetch("steps")
  ...
  test "a video has a poster, and an audio file does not pretend to" do
    ...
    Tenant.switch(@tenant) do
      bytes = Thumbnail.for(reference("clip.mp4"), size: "small")
