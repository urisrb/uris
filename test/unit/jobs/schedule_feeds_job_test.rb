require "test_helper"

class ScheduleFeedsJobTest < ActiveSupport::TestCase
  include ActiveJob::TestHelper

  setup do
    @tenant = Tenant.create!(subdomain: "feeds-#{SecureRandom.hex(4)}", name: "Feeds")
    @other = Tenant.create!(subdomain: "feeds-#{SecureRandom.hex(4)}", name: "Elsewhere")
  end

  def schedule(tenant, **attrs)
    Tenant.switch(tenant) do
      feed = Feed.create!(type: Feed::ADDRESS, key: "/watch-#{SecureRandom.hex(4)}")

      feed.create_schedule!({ prompt: "anything new?" }.merge(attrs))
    end
  end

  test "a feed whose next run has come round is started" do
    due = schedule(@tenant, interval: 1.hour.to_i, next_run_at: 1.minute.ago)

    assert_enqueued_with(job: AnalyzeFeedJob,
                         args: ->(args) { args.first(2) == [ @tenant.id, due.feed_id ] }) do
      ScheduleFeedsJob.perform_now
    end

    Tenant.switch(@tenant) do
      analysis = Analysis.newest_first.first

      assert_equal "schedule", analysis.cause
      assert_equal due.feed_id, analysis.feed_id
    end
  end

  test "a feed whose next run is still ahead is left alone" do
    schedule(@tenant, interval: 1.hour.to_i, next_run_at: 1.hour.from_now)

    assert_no_enqueued_jobs(only: AnalyzeFeedJob) { ScheduleFeedsJob.perform_now }
  end

  test "a feed with no interval only ever runs when asked" do
    schedule(@tenant, interval: nil, next_run_at: 1.minute.ago)

    assert_no_enqueued_jobs(only: AnalyzeFeedJob) { ScheduleFeedsJob.perform_now }
  end

  test "a paused feed is skipped however overdue it is" do
    schedule(@tenant, interval: 1.hour.to_i, next_run_at: 1.day.ago, paused_at: Time.current)

    assert_no_enqueued_jobs(only: AnalyzeFeedJob) { ScheduleFeedsJob.perform_now }
  end

  test "scheduling reaches every tenant rather than the one that happened to be current" do
    mine = schedule(@tenant, interval: 1.hour.to_i, next_run_at: 1.minute.ago)
    theirs = schedule(@other, interval: 1.hour.to_i, next_run_at: 1.minute.ago)

    assert_enqueued_jobs 2, only: AnalyzeFeedJob do
      ScheduleFeedsJob.perform_now
    end

    Tenant.switch(@tenant) { assert_equal mine.feed_id, Analysis.newest_first.first.feed_id }
    Tenant.switch(@other) { assert_equal theirs.feed_id, Analysis.newest_first.first.feed_id }
  end

  test "starting a feed moves its next run an interval out, so one tick cannot start it twice" do
    due = schedule(@tenant, interval: 1.hour.to_i, next_run_at: 1.minute.ago)

    assert_enqueued_jobs 1, only: AnalyzeFeedJob do
      2.times { ScheduleFeedsJob.perform_now }
    end

    Tenant.switch(@tenant) do
      assert_in_delta 1.hour.from_now, due.reload.next_run_at, 5
    end
  end
end
