...
class AnalyzeThingJobTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    Tenant.switch(@tenant) do
      ...
      @thing = Thing.create!(kind: "text", title: "notes.txt")
      ThingReference.record!(thing: @thing, resource: @storage,
  ...
  test "asking for an analysis opens a run before the job is enqueued" do
    ...
    assert_enqueued_with(job: AnalyzeThingJob) do
      Tenant.switch(@tenant) { run = AnalyzeThingJob.start!(@tenant.id, @thing.id) }
    end

    Tenant.switch(@tenant) do
      assert_equal "analyze", run.kind
      assert_equal "queued", run.status
      assert_equal({ "id" => @thing.id }, run.selector)
    end
  end

  test "the run finishes when the analysis does, and counts the thing it read" do
    run = Tenant.switch(@tenant) { AnalyzeThingJob.start!(@tenant.id, @thing.id) }

    perform_enqueued_jobs(only: AnalyzeThingJob)
    ...
    Tenant.switch(@tenant) do
      ...
      assert @thing.reload.analyzed_at.present?
    end
  end
  ...
  test "a retry carries its run rather than opening a second one" do
    ...
    enqueued = enqueued_jobs.find { |job| job["job_class"] == "AnalyzeThingJob" }

    assert_equal [ @tenant.id, @thing.id, run.id ], enqueued["arguments"]
  end

  test "a run whose thing has gone away is closed rather than left open" do
    run = Tenant.switch(@tenant) { AnalyzeThingJob.start!(@tenant.id, @thing.id) }
    Tenant.switch(@tenant) { @thing.destroy! }
  ...
  test "bytes that may come back leave the run open while the job retries" do
    ...
    assert_enqueued_with(job: AnalyzeThingJob) do
      AnalyzeThingJob.perform_now(@tenant.id, @thing.id, run.id)
    end

    Tenant.switch(@tenant) { assert run.reload.open?, "a run being retried is not finished" }
  end

  test "giving up on a thing closes its run with the reason" do
    run = Tenant.switch(@tenant) { AnalyzeThingJob.start!(@tenant.id, @thing.id) }

    job = AnalyzeThingJob.new(@tenant.id, @thing.id, run.id)
  ...
  test "a run marked running survives the analysis that raised under it" do
    ...
    Tenant.switch(@tenant) do
      ...
      assert_nil @thing.reload.analyzed_at
    end
  end

  test "analysis without a run still reads the thing" do
